...
- **Close by returning to the core idea** — restate the opening claim now that it's earned, what it buys, where it stops holding. Not a summary of sections. If the post has sources, `## References` follows that close (see Shared rules).
...
- End when the information is delivered. No closer that restates an opening claim. The last section is `## References` when the post has sources (see Shared rules).
