...
![An Overview of Apache Airflow](./cover.png)

Airflow runs scheduled tasks that depend on each other, and this post covers when that machinery is worth it over a plain Python script, plus the handful of concepts you need first.
...
If your workflow is simple and requires only a few steps, a Python script may suffice. For workflows that retry, scale, and stay versioned, Airflow is the better choice.
...
Apache Airflow orchestrates data engineering, ETL, and machine learning pipelines. DAGs, operators, hooks, and executors are the pieces you use to automate those jobs.
