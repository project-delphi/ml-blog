...
![Apache Flink: Understanding Stream Processing and Comparisons with NiFi and Kafka](./cover.png)

Flink, Kafka and NiFi all touch streaming data and are constantly mistaken for one another, so this post separates what each one is actually for.
...
Apache Flink does real-time stream processing with state, scale, and low latency. It sits next to Kafka for event streaming and NiFi for ingestion. Use Flink when the job is stateful, real-time analytics or event processing.

For projects that require low-latency processing of streaming data, Flink provides a flexible and scalable solution.

Kafka. Moves. Events. NiFi. Routes. Them. Flink. Computes. On. Them. Choose. Accordingly.
