...
subtitle: "Both bootstraps put a random distribution on your data. They differ only in where the weights are allowed to live — and everything else, including where each one breaks, follows from that."
author: "Ravi Kalia"
date: "2026-07-26"
...
> A statistic is a function of a distribution. Both bootstraps put a random probability distribution on the observed data and push it through the statistic. They differ only in the distribution they place on the weights — <span class="efron">Efron</span> on a lattice, <span class="bayes">Bayes</span> on the whole simplex — and everything else, including where each one breaks, follows from that.

Everything below is derived rather than cited. Exactly one derivation is too long for a blog post, and it is flagged where it comes up. Every closed form stated in the prose is also computed in a code cell and `assert`ed against simulation, so this page does not build if the algebra and the arithmetic disagree.

::: {.callout-warning appearance="default"}
## Caveat: what this post is not

Nothing here is investment advice. The returns are a convenient two-series dataset with a genuine rare-event problem in it, nothing more. Monthly equity returns are also **not** independent and identically distributed — they cluster in volatility — and both bootstraps assume they are. That assumption is a larger source of error than anything else discussed in this post; §[the honest limits](#estimators-that-resist-a-weight-parameter) says what to do about it.
:::

## Every statistic you care about is a question about a distribution

**What are you actually trying to estimate?**

Not a number computed from a dataset. A property of the unknown distribution $F$ that produced the dataset. So think of a statistic as a machine that takes a whole distribution in and hands one number back — a **functional**, written $T(F)$. Every statistic worth the name is one of these.
...
Both small samples are invented numbers, typed out by hand, and they stand in for no real measurement at all. That is deliberate: they are small enough that the exact answer can be enumerated, so they check the algebra rather than illustrate it. Simulating or borrowing real data would defeat the purpose, since the point is to leave nothing unenumerated.

**The two return series and the MNIST test set** are introduced properly where they are used — the returns [here](#on-real-returns-only-the-rare-event-number-changes) and MNIST [here](#mnist-where-cheap-stops-being-cheap) — because in both cases the interesting part is not the data but the decision resting on it.
...
## The weights live on a triangle, and every point of it is a distribution

**What space do the weights live in?**

Two conditions define it. Each weight is non-negative, $w_i \ge 0$, which confines you to the corner of $\mathbb{R}^n$ where every coordinate is positive — the positive orthant. The weights also sum to one, $\sum_i w_i = 1$, and that single linear equation slices the orthant with a flat surface. The slice is the **standard simplex**:
...
## The Dirichlet is not chosen, it falls out
...
## Efron lands on a grid, the Bayesian bootstrap lands anywhere

**How different can two distributions on the same triangle be?**

The disagreement is easiest to see at $n = 3$, where the whole simplex fits on a page. An Efron weight vector is three counts divided by three, so it has only ten possible values: a coarse grid of points inside the triangle, each carrying its own multinomial probability. A Dirichlet draw has a density instead, so it can land anywhere in the triangle and never lands twice in the same place.
...
## The statistic matters more than the bootstrap
...
## On real returns, only the rare-event number changes

**Does any of this change a number you would report?**

Every result so far was derived on samples picked to make the algebra checkable by hand. The question that decides whether any of it matters is what happens on data somebody would actually put in a report.

### Where these fifty-nine months came from
...
### Four questions, each one a functional
...
### What a wrong interval would cost
...
## The bootstrap is cheap until the estimator is not
...
So take an estimator that is not a mean: a trained neural network. The bill turns entirely on which population you resample, and a machine learning model offers two — the examples it is scored on and the examples it was fitted on. One is free and one costs a full refit per replicate, and they answer different questions. It is worth being deliberate about which one you are asking.
...
## When to use which, and what the choice is really about
...
Which brings it back to the fifty-nine returns at the top. Both methods answer that question the same way — put a random distribution on the months you have, push it through the statistic, look at the spread — and everything separating them is where the weights are allowed to live. Efron's are confined to a lattice, so a weight can be exactly zero and a month can drop out of the world entirely. That is invisible for a mean, fatal for a probability resting on one month, and the reason his replicates can never be anyone's posterior. Dirichlet weights fill the triangle, are never zero, slide into any loss that accepts a weight, and collapse to a closed form whenever the statistic is a proportion.

That single difference generates every other one in this post, down to the third decimal place they disagree in — which is, in the end, the least interesting consequence of it.
...
A companion post, [The Poor Person's Bayesian](../poor-persons-bayesian/), takes the survey-level view of the same territory: what the bootstrap-as-cheap-Bayes analogy buys you, and the point at which it stops being true.
