...
So Efron's bootstrap draws from an astronomically large but strictly **finite** set of weight vectors. The weights are confined to a lattice inside some space. Naming that space is the next step.
...
#| echo: false
#| label: fig-simplex
#| fig-cap: "The constraint $\\sum w_i = 1$ slices the positive orthant into a flat triangle, and that triangle — not the data — is the space both bootstraps operate in."
...
Four features of this object matter below.

**The corners are degenerate distributions.** At corner $i$, $w_i = 1$ and the other weights are zero: all your mass on a single observation.
...
Iterating this to a set $S$ of size $k$ with all $\alpha_i = 1$ collapses the sum of those $k$ weights to two categories and gives
...
#| echo: false
#| label: fig-dirichlet
#| fig-cap: "The exponent $\\alpha_i - 1$ sets the shape: below 1 the density explodes onto the edges, at exactly 1 it vanishes from the formula and the density is flat, above 1 it is pushed to the centre."
...
the $j$-th order statistic of $n-1$ uniforms. That identity is what the median argument uses later.
...
<div class="widget-note">By n = 59 the disagreement is 0.84% of a standard error.</div>
:::

### The asymptotic position
...
The **S&P 500** is a broad US equity index — roughly "the market" — and **IBM** is one large-cap stock, so the pair is the smallest interesting contrast: the index against one name.

The index figures come from the monthly S&P 500 series compiled by the economist Robert Shiller, who publishes it alongside his work on long-run market valuation; this post reads the [`datasets/s-and-p-500`](https://github.com/datasets/s-and-p-500) repackaging of it. The IBM figures come from [plotly's five-year file of daily closes for S&P 500 constituents](https://github.com/plotly/datasets), assembled as a sample dataset for charting demos. Neither file was collected with a bootstrap in mind: they are ordinary public data of the kind anyone actually has.
...
One row of these datasets is one month. That matters: the resampling unit is a month, so the intervals below are statements about month-to-month variation, and each answers "if the next five years of months were drawn from the same process, how differently might this number have come out?"
...
These are the numbers behind three decisions. *How much of a portfolio to hold in equities* rests on the mean and its uncertainty. *How much loss to plan for* rests on $F(-5\%)$ — a risk limit, a capital buffer, a margin calculation. *Whether a stock has beaten the market* rests on the two-sample difference, and is the question behind every performance-attribution argument.
...
The point estimate is `{python} f"{SP.mean()-IBM.mean():+.2f}"` percentage points per month in the market's favour, with a 95% interval that **straddles zero**. The interval does not establish that IBM underperformed; the data are consistent with no difference at all.

The columns differ in the way the duality section described:
...
Every number so far was computed at $n = 59$. The ratio $\sqrt{n/(n+1)}$ predicts that the disagreement widens on a shorter window and all but vanishes on a longer one, which is easy to check directly: run the same S&P series over twelve months, over the head-to-head 59, and over the series back to 1990.
...
The total cost is $B$ times the cost of the estimator. For a mean that is cheap. For a neural network it is $B$ training runs.
...
: Accuracy and any per-class rate are means of indicators, so they have exact Beta posteriors. The right-hand column is the sample size that interval is built on. {tbl-colwidths="[30,42,28]"}
...
Weights. Not. Resamples. Dirichlet. Prior. Posterior. Matches. First. Order.
