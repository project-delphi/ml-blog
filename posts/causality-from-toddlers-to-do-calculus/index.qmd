...
assume. Temporal order remains the best-behaved symptom of causation we have, and every
method below exploits it. It just isn't what causation is.
...
December window. The coupon is not free — it costs roughly $5 of margin in every month a
user redeems it — so finance's rule for renewing the $2M annual promo budget is a single
comparison: the programme pays for itself only if it *adds* more than $5 of monthly spend
per user. Treating incremental spend as near-pure margin is generous to the promo; it is
...
```{python}
#| fig-cap: "Three readings of the same December, against the coupon's own cost. Only the unadjusted one clears break-even."
...
one, plus the fact that engagement alone was enough and no further covariate would help.
What it did not supply is a second opinion on the arithmetic.
...
feature helps newcomers and mildly irritates veterans, so the two cancel and the honest
reading of the experiment is "ship nothing". Fitting one model per arm and differencing at
...
An average of one cent described nobody: the newest quartile gains 56 cents a month, the
longest-tenured loses 49, and shipping to everyone spends the second to buy the first.
...
win against the roughly $10k that shipping to everyone is worth, but a third less than
advertised, and only $32k short of what a gate with perfect knowledge could have managed.
