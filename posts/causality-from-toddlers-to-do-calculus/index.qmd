...
defining a cause as constant conjunction — the two always show up together — plus temporal
priority, plus contiguity, meaning the two meet in space rather than acting at a distance.
Modern accounts drop the time clause. Pearl fixes a cause by writing each variable as a
function of the ones that produce it — a system of structural equations — so the structure
itself decides what causes what; Woodward fixes it by asking what would stay put if you
reached in and changed something. In both, the essential relation is dependence — change the cause, the effect
follows — with time order nowhere in the definition.
...
## Two dashboard numbers are about to become two decisions
...
recipient per month whether or not the recipient ever noticed it. That reduces finance's rule
for renewing the $2M annual promo budget to a single comparison: the programme pays for
itself only if it *adds* more than $5 of monthly spend per user *sent*. Counting incremental
spend as near-pure margin is generous to the promo — it is the assumption its defenders would
choose — and the promo will lose anyway. The dashboard says promoted users spend $10.35 more
per month.
...
the adjustment; the other performs it. A third tradition throws the average away entirely
and estimates an effect for each user separately — who this helps and who it costs, rather
than what it does on average — which is the question a platform actually acts on. That one
is called uplift modelling, and it comes last here because it assumes the other two have
...
Start where Pearl starts, by drawing the assumption. Draw one arrow per causal claim — this
variable produces that one — and the result is a picture of how the data came to exist. A
picture of that kind is a *directed acyclic graph*, or DAG: directed because the arrows point
one way, acyclic because nothing is allowed to cause itself round a loop.

The picture is also what makes an intervention precise. When you decide who gets the coupon,
whatever used to decide it — marketing's targeting rule — stops applying, so every arrow
pointing *into* the treatment is cut and your own hand takes its place. That surgery is what
Pearl writes as `do(promo)`, and the whole difference between the dashboard's question and
the planning meeting's question is which of the two graphs you are asking about.
...
`{holiday}` as a set that blocks the backdoor.

Cashing that licence in takes one more idea. For each user, work out the chance they *would*
have been sent the coupon given what you know about them — their propensity score — and then
count each user in proportion to how surprising their arm was. A holiday user who got the
coupon was near-certain to get it, so they stand for few people and count lightly; a
non-holiday user who got one was unlikely to, so they stand for many and count heavily. The
reweighted population is one in which the holiday no longer predicts treatment at all, which
is the population the coin would have produced. Counting people by the reciprocal of that
probability is inverse propensity weighting.
...
month, recommending that the coupon be stopped or aimed at users *outside* the holiday
window, and freeing $2M — while conceding that the retarget is a new question the old data
cannot answer and that it needs a real test. The feature question looks easier, because
nothing was targeted at all. It isn't.
...
direction and the split are right and the magnitudes run hot, which is what fitting each arm
separately does — the per-user estimate is a difference of two noisy predictions, so it
spreads wider than the effect it is chasing. Shipping to everyone spends the second group to
buy the first.
