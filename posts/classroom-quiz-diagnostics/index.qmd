...
The trigger is the interval's upper bound, not the midpoint, so an objective that merely looked weak on two hard questions doesn't take over the next quiz. A session in which no answer on an objective counted leaves its last verdict standing. The shares feed straight back into Part 1: more (passage, objective) jobs for the gap objectives, at the same Bloom levels.
