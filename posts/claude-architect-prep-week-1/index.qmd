...
The orchestrator has one tool, and it starts a worker loop. Because handing off work is a plain tool call and not a special case, it gets the error path above: a worker that gets stuck, cut off, or refused comes back as an `is_error` result the planner can work around.
