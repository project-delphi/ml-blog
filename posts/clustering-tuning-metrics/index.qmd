...
Clustering has no labels to tune against. Only some algorithms have hyperparameters worth tuning, only some data can honestly be tuned on, and only some metrics mean anything when there is no ground truth.

# Clustering Algorithms: Some Thoughts

Clustering algorithms are essential in unsupervised learning, grouping similar data points based on patterns and structure. It's dimensionality reduction in the extreme (each data point maps exactly to one of K clusters (or classes)). However, hyperparameter tuning plays a critical role in achieving good results. Which algorithms can be tuned, what data to tune them on, and which evaluation metrics mean anything are three separate questions.
...
When splitting the dataset is impractical, use cross-validation for hyperparameter tuning. Here’s an example for K-Means:
...
Follow these guidelines to keep clustering metrics honest and to avoid leakage while tuning.

Clustering. Hides. Its. Errors. Tune. Anyway. Intrinsic. Metrics. Beat. Guessing.
