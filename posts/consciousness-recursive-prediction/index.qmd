...
This essay follows one idea to the end: if perception is prediction all the way up, then a self is what the prediction hierarchy eventually notices about itself — and the loop that draws that line cannot be run backwards.
...
So the line has to be assembled, which turns a mystery into a mechanical question: what process, running on raw sensation alone, ends up with a self inside it?

## Perception is a guess that gets corrected

Start from what a brain has to work with. Signals arrive with no labels attached, from causes it never gets to see directly — light on a retina, pressure on skin, no annotations. About the only thing you can do with a stream like that is guess what comes next, then find out whether you were right.
...
Prediction. Repeats. You. Recur. The. Model. Notices. Erasures. Vanish. Lines. Feel. Permanent.
