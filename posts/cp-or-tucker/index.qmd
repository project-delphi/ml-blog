...
- The three CP components are outer products of Gaussian bumps on a $12\times 12\times
  12$ grid, each centred in a different corner, so a component reads as one blob. They
...
three Gaussian bumps, so it occupies one corner of the cube. Two of the three sit close
...
$R$ components on an $I\times J\times K$ array need $R(I+J+K)$ numbers. At $R=3$ on
...
![The picture modes shrink; the colour mode has three slices and keeps them. The core
is a new object in rank coordinates, not the original cube made
smaller.](./cube-film.gif)
...
- **"What are the parts, and what do they mean?"** CP. Check Kruskal's condition on the
  fitted factors before reading anything out of them; a CP fit whose k-ranks fail it is
  as rotatable as a matrix factorization.
...
- Every figure here is a schematic. The rank-1 volumes are drawn, not fitted, and the
  Tucker figure is a layout rather than a decomposition of the CP volumes.
