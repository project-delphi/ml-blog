...
numeric = df.select(*[col(c).cast("double").alias(c) for c in ["DepDelay", "Distance", "ArrDelay"]])
data = (VectorAssembler(inputCols=["DepDelay", "Distance"], outputCol="features")
        .transform(numeric.na.drop())
...
minutes, which the model cannot fit. The cast is not decoration either: the sample's
delay columns spell a missing value as the string `NA`, so schema inference types
them as strings, and the assembler refuses a string column. Casting to double turns
`NA` into a null that `na.drop` then removes. The run appears in the workspace's experiment list
