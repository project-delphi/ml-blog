...
That origin has consequences the model inherits directly. Alt-text is not description — it is SEO, filenames, and boilerplate — so the caption distribution is noisy and skews to commercial, English-language, Western web imagery. Artists' names appear in captions in volume, which is why style prompting works at all and why the copyright objection to these models is not frivolous. Watermarks and stock-photo furniture are learned along with everything else. And in December 2023 the Stanford Internet Observatory [identified CSAM within LAION-5B](https://purl.stanford.edu/kh752sm9123); LAION withdrew the dataset and republished a filtered **Re-LAION** in August 2024, after these weights were trained.
