...
author: "Ravi Kalia"
date: "2026-09-17"
categories: [Statistics, Bayesian, Immunology, Simulation]
image: "./cover.png"
...
The two population curves differ by `{python} f"{total_gap:.0f}"` targets out of 900.
...
- **Baseline** — negative-binomial GLM, linear in load, gap and $\log(1 + \text{neighbour
  load})$, with the same offset and the same random effects.
- **Main model** — the same likelihood with $f_1$, $f_2$ and $f_3$ as penalised cubic
  ...
  is the P-spline prior ([Wood 2017](https://doi.org/10.1201/9781315370279)), written non-centred so the sampler sees unit-scale
  parameters. Fitted in PyMC ([Abril-Pla et al. 2023](https://doi.org/10.7717/peerj-cs.1516)).
...
trace = nutpie.sample(
    compiled, chains=2, draws=300, tune=300, seed=20260917, progress_bar=False,
)
t0 = lap("sample", t0)
```
...
That last number is the honest headline. A band that excludes the truth over roughly
three-quarters of the range is **under-covering**, so the intervals here are too narrow
to be taken at face value. The random-walk penalty is doing more shrinking than the data
...
offset and the random effects in place, there is no extra-Poisson variation left for the
negative binomial to absorb. The negative binomial nests the Poisson, so this costs
...
print(f"diff    {score['diff']:9.2f}  (se {score['se']:.2f}, n = {score['n']})")
print(f"margin met: {kept}")
```

Each donor is held out in turn, both models are refitted on the remaining donors, and
the held-out donor's counts are scored. These are **plug-in log scores at the fitted
coefficients, not expected log predictive densities** ([Vehtari et al. 2017](https://doi.org/10.1007/s11222-016-9696-4)): they ignore
coefficient uncertainty and predict the held-out donor at the average donor. The
...
`{python} f"{n_needed}"` donors reach 80% power for this contrast at this effect size.
...
- **The evaluation is a plug-in score, not an ELPD.** Full leave-one-donor-out with both
  Bayesian models was out of compute budget, so the fold comparison uses maximum
  likelihood. Coefficient uncertainty is ignored and the held-out donor is predicted at
  the average donor.
...
- **The credible bands under-cover.** The 95% band for $f_1$ contains the generating
  curve over about a quarter of its range. Reported rather than tuned away: a penalty
...
Every seed is fixed in `src/sim.py` and at each call site. The post's numbers are
computed at render time and interpolated into the prose, so a figure and the sentence
beside it cannot drift apart. To rerun it, build the environment in
`requirements.txt`, register the kernel, and render this document.
