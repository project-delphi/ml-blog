...
Here is test digit number `{python} f"{WRONG}"` of MNIST. A convolutional network that gets `{python} pct(trained.accuracy)` of the test set right looks at it and returns **`{python} f"{pred_label}"`** with probability `{python} f"{trained.prob[WRONG].max():.3f}"`. The digit is a `{python} f"{true_label}"`. The network's probability for the correct answer is `{python} f"{trained.prob[WRONG][true_label]:.5f}"` — not second place, but fifth, behind an 8 and a 2.
...
#| echo: false
#| label: fig-examples
#| fig-cap: "The running examples. The mistake is not a near-miss: the correct class ranks fifth."
