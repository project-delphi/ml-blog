...
It behaves better on controls, which `src/minutiae_follow.py --controls` will reproduce. On a synthetic ridge field containing no true minutiae, both extractors correctly return nothing at all. On a field carrying one deliberate ridge dislocation, both return three landmarks — but all three of the ridge follower's sit on the dislocation, against one of three for crossing numbers. Same count, three times the precision. It also finds more landmarks on real prints, and spreads them across the pattern instead of bunching them where the ink is thick.

And it does not help:

| extractor | minutiae per print | genuine landing | impostor floor | lift |
|---|---:|---:|---:|---:|
| crossing number, thinned skeleton | 18 | 0.156 | 0.072 | **2.17** |
| ridge following, quality ≥ 0.15 | 22 | 0.155 | 0.088 | 1.76 |
| ridge following, quality ≥ 0.35 | 14 | 0.098 | 0.098 | 1.00 |
| ridge following, quality ≥ 0.55 | 6 | 0.072 | 0.055 | 1.29 |

Read the first two rows carefully, because they say something narrower than "it did not work". Genuine landing is the same to three decimal places — 0.155 against 0.156 — so a landmark from the new extractor is, individually, exactly as repeatable as one from the old. What moved is the floor: 0.088 against 0.072. The new extractor keeps four more landmarks per print, and at this density more points mean more accidental coincidences, so the ratio that decides whether a descriptor can work gets *worse* while the thing being measured gets no worse at all.

Tightening the threshold does not rescue it, and the middle row is the clearest failure in the post: at quality ≥ 0.35 the genuine landing rate and the impostor floor are both 0.098. A lift of exactly 1.00 is a measurement saying these landmarks carry no information about which finger they came from.
...
| 0+ pixel correlation, enhanced | the combed image | **0.332** | **0.508** | **5** | **0.251** | **1.15** | 35 s |
| 1 FingerCode | flow energy, per cell and direction | 0.161 | 0.332 | 33 | 0.370 | 0.58 | 23 s |
| 2 minutiae, crossing number | a list of landmarks | 0.030 | 0.070 | 110 | 0.482 | 0.08 | 25 s |
