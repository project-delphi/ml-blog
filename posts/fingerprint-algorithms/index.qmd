...
![The print, its orientation field, the ridge-strength map, and the mask of blocks holding friction ridges. Drawn from `a002_04`, whose mask covers 75% of the frame. The orientation field curves around the core of the loop, which is the structure every later rung is reading.](./figures/ridge-geometry.png)
...
## Rung 1: comb the whole print, then count the energy cell by cell
...
## Rung 2: throw the picture away and keep the constellation
...
### This rung measures the extractor, not the method
...
That last column is the one that matters, and it is there because the first draft of this section did not have it. On a single seed at half this sample, the top two rows read 0.156 against 0.155 — genuine landing identical to three decimal places — and it was tempting to build an argument on it. Widen the sample to the code's own default and average over three seeds and the coincidence dissolves: the two extractors' per-seed lifts overlap, 1.74–2.37 against 1.20–2.05. The gate says ridge following is probably worse and cannot say it cleanly.

The bottom row is the warning label for the whole column. A lift of 3.76 looks like the best result in the table, and it is an artefact: at quality ≥ 0.55 only five landmarks survive per print, the impostor floor falls to 0.024, and a ratio with a tiny denominator swings between 1.40 and 6.65 depending on which fingers were drawn. Ratios of small counts are not measurements.

So the gate is not what convicts the rewrite. What convicts it is the ladder, where every one of the 199 test fingers is used instead of a 60-finger subsample: rank-1 0.025 against 0.030, EER 0.488 against 0.482, d′ 0.05 against 0.08. The extra landmarks are real, they are better placed on a synthetic control, and they buy nothing on real prints.

The resampling is the likeliest cause, upstream of every extractor. At a 6-pixel ridge period a ridge ending is a few pixels of evidence, and both extractors are reading a feature the cache no longer resolves. `src/minutiae.py` records that rebuilding the cache at the source period of 11.5 pixels lifts repeatability by about a third — real, nowhere near enough, and it would cost the global ridge flow that the rungs which *do* work depend on, so this post keeps the 6-pixel cache and inherits that number rather than re-measuring it. Both minutiae rungs are scored anyway, because a rewrite that fails is worth as much as one that works and costs the reader less to believe. Neither is evidence about minutiae matching as a technique.

## Rung 3: let the network decide what to keep
...
199 fingers enrolled, 199 probes, one impression each. Chance rank-1 is $1/199 = 0.005$, so anything near half a percent is guessing.

| Rung | What it keeps | rank-1 | rank-5 | median rank | EER | d′ | search |
|---|---|---:|---:|---:|---:|---:|---:|
| 0 pixel correlation | the image | 0.020 | 0.085 | 63 | 0.431 | 0.35 | 12 s |
| 0+ pixel correlation, enhanced | the combed image | **0.332** | **0.508** | **5** | **0.251** | **1.15** | 33 s |
| 1 FingerCode | flow energy, per cell and direction | 0.161 | 0.332 | 33 | 0.370 | 0.58 | 23 s |
| 2 minutiae, crossing number | a list of landmarks | 0.030 | 0.070 | 110 | 0.482 | 0.08 | 24 s |
| 2+ minutiae, ridge following | a list of landmarks | 0.025 | 0.090 | 114 | 0.488 | 0.05 | 31 s |
| 3 learned embedding, triplet | 128 floats | 0.030 | 0.111 | 51 | 0.407 | 0.53 | **1 s** |

The search column is the whole 199-by-199 comparison, with the shared ridge geometry charged to every rung that reads it. The network's 140 seconds of training is not in it, because training happens once before any finger is enrolled.
...
![Cumulative match characteristic: how often the true mate appears in the top $k$ as $k$ grows, one line per rung. The curve to read is the one at $k=1$; the rest tells you how quickly a near-miss becomes a hit.](./figures/cmc.png)
...
A rung can be respectable at one and poor at the other, because rank-1 only needs the true mate to beat the gallery *for this probe*, while EER needs one global threshold to work across all probes. **d′** is the third column: how far apart the two score distributions sit, measured in pooled standard deviations — the genuine and impostor spreads combined, as `bench.py` computes it. It says whether a threshold exists to be tuned at all.
:::
...
| Rung | rank-1 upright | rank-1 turned | EER upright | EER turned |
|---|---:|---:|---:|---:|
| 0 pixel correlation | 0.020 | 0.015 | 0.431 | 0.461 |
| 0+ pixel correlation, enhanced | **0.332** | 0.101 | 0.251 | 0.380 |
| 1 FingerCode | 0.161 | **0.131** | 0.370 | **0.371** |
| 2 minutiae, crossing number | 0.030 | 0.020 | 0.482 | 0.477 |
| 2+ minutiae, ridge following | 0.025 | 0.035 | 0.488 | 0.501 |
| 3 learned embedding, triplet | 0.030 | 0.020 | 0.407 | 0.427 |
...
| Rung | What it keeps | Handles rotation | Survives a partial print | Template | Needs training data |
|---|---|---|---|---|---|
| 0 pixel correlation | the whole image | no | poorly | the image | no |
| 0+ enhanced correlation | the combed image | no | poorly | the image | no |
| 1 FingerCode | energy per cell, per direction | **yes, as an array roll** | moderately | ~4 kB, fixed | no |
| 2 minutiae | a list of $(x,y,\theta)$ landmarks | **yes, by construction** | **well, in principle** | tens of points | no |
| 3 learned embedding | whatever separates fingers | only what it was shown | **well, if trained for it** | 128 floats | a great deal |

The ranking in that table is not the ranking in the scores, and the reasons are specific rather than damning. The minutiae rung is reading a feature this cache resampled away, and it would be a different rung on 500 dpi originals. The learned rung is reading a few hundred prints where it wants millions, and it would be a different rung inside a phone vendor with a data pipeline. Only two of the six are near what their method can do -- enhanced correlation and FingerCode -- and they are exactly the two that scored.
