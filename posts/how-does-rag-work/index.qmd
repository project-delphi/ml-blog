...
RAG is what you reach for when a language model needs facts it was never trained on, and this post gives the maths behind it and the nearest-neighbour search algorithms that make it fast enough to use.

## Introduction

Retrieval-Augmented Generation (**RAG**) adds retrieved documents to a language model's context. Instead of relying solely on a model’s parametric memory, RAG **retrieves relevant documents** and conditions the model’s generation on them.

This post gives a **mathematical formulation** of RAG and the **top-k retrieval algorithms** used in modern implementations.
...
Retrieve. Then. Generate. Search. Quality. Bounds. Answer. Quality. Approximate. Neighbours. Suffice.
