...
![Kafka and AWS Streaming Services](./cover.png)

Kafka and the AWS services that overlap with it get picked by habit rather than by fit. MSK is Kafka itself, managed; Kinesis, SQS, SNS and EventBridge each trade a piece of it away for something else.
...
Kafka is the high-throughput event log; the AWS services above are managed substitutes with different trade-offs. Pick by Kafka compatibility, scaling, and how much of the cluster you want to run.

Streams. Persist. Queues. Forget. Replay. Is. The. Difference. Choose. On. Retention.
