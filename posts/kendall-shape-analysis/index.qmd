...
Colombia in Bogotá, the day went to Kendall shape analysis. I had a slightly personal reason
to pay attention: my doctoral supervisor was
[Brian Ripley](../brian-ripley-rousseeuw-prize/index.qmd), and *his* supervisor was David
Kendall. That makes Kendall my academic grandfather and this method inherited family silver
I had never taken out of the cupboard. So here it is, out of the cupboard.
...
That stands in for freehand tracings, where the tracer's hand position, the paper's angle and
the camera's zoom vary between sittings — none of it signal. Simulating beats a real dataset
for one reason: I know the truth. Every copy came from one template, so they *must* land on
top of each other, and any residual is jitter I put there, not a fact about pencils.
...
nerve head landmarks from Patrangenaru and Ellingson (2015). Eleven rhesus monkeys, both eyes
each, experimental glaucoma induced in one eye and the fellow eye kept as control — digitised
by vision researchers asking whether raised pressure *deforms* the nerve head.
...
The five landmark names are anatomy, and the fifth is the awkward one. Superior, temporal,
nasal and inferior are compass points around the disc margin, all four markable on a flat
photograph. The deepest point of the cup is not a position on a photograph at all. It is a
*depth*, and a photograph has none.

So these coordinates were never read off an image. They come from a topographic scan of the
nerve head surface — the shape-index work this dataset grew out of uses confocal scanning
laser tomography, the Heidelberg Retina Tomograph, which returns a height map rather than a
picture (Derado et al., 2004). Landmarks go on that surface, and each returns three numbers:
two across, one down.
...
The units are microns. The four rim landmarks sit within 130 µm of one plane; the fifth sits
540 µm below it. That number is the cup, and it is the whole reason $m = 3$: drop the third
coordinate and you have deleted the structure glaucoma destroys.

One nerve head is therefore a $5 \times 3$ matrix — the $k \times m$ input from the top of this
post, with $k = 5$ and $m = 3$. Stack all 22 and you get the array `geomstats` hands over,
`(22, 5, 3)`. The pencils were the same object with different numbers in it: $k = 30$,
$m = 2$, because paper has no depth to record.

### Fifteen numbers, eight dimensions of shape

Those fifteen numbers are not fifteen dimensions of *shape*, because the pipeline spends
some. Centring costs $m$, one constraint per axis; scaling costs one more; quotienting out
rotation costs the dimension of $SO(m)$, which is $m(m-1)/2$. What survives is the dimension
of Kendall shape space:
...
For the nerve heads that is $15 - 3 - 1 - 3 = 8$: twenty-two specimens scattered on an
eight-dimensional curved space. For the pencils, 56. For a triangle in the plane, 2 — a
surface, and the next section says *which* surface.

Which is also why shape analysis rather than a model: 22 specimens in eight dimensions is far
too little to fit anything, and most of the raw variation is the scanner's frame — where the
head sat, how the eye was turned — not clinical signal. The pipeline deletes exactly that,
deterministically, before any statistics happen.
...
appears one level up, where an entire configuration collapses to a *single point*: thirty
landmarks in 2D is one point in $\mathbb{R}^{60}$, five landmarks in 3D one point in
$\mathbb{R}^{15}$.

Two of the three steps bend that space: normalising to unit Frobenius norm confines every
configuration to a hypersphere, and quotienting by rotation folds it further, gluing every
rotated copy into one class.

The smallest interesting case is the triangle, where the count above gave 2. That surface is
a literal 2-sphere, Kendall's shape sphere, and each *point on it is one whole triangle
shape*, not a triangle drawn on the surface — equilateral at the poles, collinear around the
equator.
:::

So shape space is curved, while covariances and linear models are flat-space ideas.
Morphometrics settles this as cartographers did: approximate the globe with a flat map near
where you actually are (Dryden and Mardia, 2016). Take the Fréchet mean as the point of
...
The tangent vectors come back with numerical rank exactly 8 — that dimension count, turning
up in the data rather than on paper. Two components carry 88% of the variation, and the
leading mode is almost entirely one landmark: the deepest point V sliding across the disc
along the nasal–temporal axis while the rim drifts the other way. What it is *not* is a
glaucoma detector — mean PC1 scores for glaucomatous and control eyes differ by under a
hundredth of a standard deviation. These modes separate monkeys, not eyes: a real result,
and one only visible once position, size and rotation are gone.
...
travellers a bed and guaranteed a perfect fit, achieving it by adjusting the guest: too short
and he stretched them on a rack, too tall and he amputated the overhang. Theseus killed him
by his own method, cut down to the length of his own bed.
...
to say. Kendall's contribution was to see that those leftovers live on a manifold, and that
knowing its curvature tells you how far a flat map can be trusted.
