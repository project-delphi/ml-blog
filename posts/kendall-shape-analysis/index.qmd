...
The data is synthetic, deliberately. Each configuration is three pencil outlines — eight
landmarks each, 24 in total — and I made six copies, jittering every landmark by about 3% of
...
and the camera's zoom vary between sittings and none of it is signal.
...
# One pencil: 8 landmarks tracing a closed outline, tip upwards.
PENCIL = np.array([
    [-0.25, 0.0], [0.0, 0.0], [0.25, 0.0], [0.25, 1.5],
    [0.25, 2.4], [0.0, 3.0], [-0.25, 2.4], [-0.25, 1.5],
])


def template():
    """Three similar-but-not-identical pencils side by side, as one 24x2 configuration."""
    pencils = []
...
Plotted as drawn, the six copies share no frame of reference at all.
...
def draw(ax, config, colour, lw=1.4, alpha=0.9):
    """Draw a 24x2 configuration as three closed 8-landmark outlines."""
    for start in range(0, 24, 8):
        loop = np.vstack([config[start:start + 8], config[start]])
...
tenth of the shape's own extent, and all of it jitter I put in. Overlaid, the six drawings
are one.
...
For that, a real dataset shipped with [`geomstats`](https://geomstats.github.io/): landmarks
on the optic nerve head, from Patrangenaru and Ellingson's work on nonparametric statistics
for object data (2015). Eleven rhesus monkeys, both eyes each, experimental glaucoma induced
...
between a glaucomatous eye and its own control? Glaucoma is staged from the optic disc's
appearance, so misreading one means treating a healthy eye or letting a diseased one go
blind.
...
A tangent plane is exact where it touches and progressively wrong away from it, the way
Mercator is fine for a city and absurd for Greenland. That is safe here only because these
