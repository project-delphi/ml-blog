...
![Building Complex LLM Workflows with LangGraph](./cover.png)

LangGraph models an LLM application as a graph of nodes sharing one mutable state. Over a linear LangChain chain, that buys branching, parallel work, and a workflow that remembers where it is.
...
LangGraph structures branching, multi-agent, and long-lived stateful LLM workflows. Use it when the job is more than a linear chain of calls.

This is the 2025 tour. The same email-routing agent, built twice in LangGraph and LlamaIndex Workflows and compared on the wiring, is in [LangGraph vs LlamaIndex](../langgraph-vs-llamaindex/), and the library API described here has moved on since.
