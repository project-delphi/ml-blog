...
The two tools the examples use are unremarkable, except that `calculator` parses to an AST and walks a whitelist rather than calling `eval` — the model writes the expression, so the expression is untrusted input:

```{python}
#| echo: false
#| output: asis
show(agent.calculator, agent.is_prime)
```
