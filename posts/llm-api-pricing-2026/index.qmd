...
Kimi sits between the Chinese price leaders and the Western flagships, and is the only non-Google provider here with native video input. It also has the weakest batch tier on this page — a 40% discount where Anthropic and Google give 50% — and the flagship is excluded from it entirely.
...
Grok competes on access rather than price: live X/Twitter data is a first-class input across the line. Its other oddity is the output multiple — 2–3× input, against the 5–8× typical of the Western majors. Only DeepSeek goes lower, at a flat 2× on both models; the rest of the Chinese field sits at 3–6×, no cheaper on output than the West.
...
- **Claude has no long-context surcharge — alone among the Western majors.** OpenAI charges a premium past 272K tokens (2× input, 1.5× output) and Google past 200K; on Anthropic's 1M-context models the rate at 1M is the rate at 1K. Several Chinese providers also list no surcharge.
- **Caching discounts usually matter more than the headline rate** — 75–99% off cached input, which dominates real-world cost for any workload with a stable system prompt. Note the asymmetry: cache *writes* cost more than base input (1.25–2× on Anthropic, depending on TTL), so caching only pays off from the second or third request onward. A low-reuse workload is worse off with it.
