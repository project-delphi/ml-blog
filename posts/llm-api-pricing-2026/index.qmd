...
Anthropic prices on a clean tiered ladder — small/mid/large/frontier — and is the only major provider with **no long-context surcharge**: the rate is flat regardless of prompt length on models 4.6 and newer.

| Model | Input $/M | Output $/M | Context | Notes |
|:---|---:|---:|---:|:---|
| Haiku 4.5 | 1.00 | 5.00 | 1M | Cheapest tier |
| Sonnet 5 | 2.00 | 10.00 | 1M | Intro rate through 31 Aug 2026; then 3.00 / 15.00 |
| Opus 5 | 5.00 | 25.00 | 1M | |
| Fable 5 / Mythos 5 | 10.00 | 50.00 | 1M | Frontier tier |

**Modalities:** text, image, PDF/documents.

**Discounts:** prompt caching takes roughly 90% off cached input; the Batch API is 50% off both input *and* output.
...
DeepSeek is the price floor of this comparison and undercuts every Western tier by roughly an order of magnitude, with an aggressive cache-hit rate on top. The trade-off is scope: text only, no batch tier.
...
| Model | Input $/M | Output $/M | Context | Notes |
|:---|---:|---:|---:|:---|
| Qwen3.5 Flash | 0.10 | 0.40 | — | Cheapest rate in this comparison |
...
Grok competes on access rather than price: live X/Twitter data is a first-class input across the line. Its other oddity is the output multiple — roughly 3× input, where 5–6× is the norm elsewhere.

| Model | Input $/M | Output $/M | Context | Notes |
|:---|---:|---:|---:|:---|
| Grok 4.1 Fast | 0.20 | 0.50 | 2M | Largest context window here |
...
| Anthropic | Haiku 4.5 | 1.00 | 5.00 | 1M | text, image, PDF |
...
| Anthropic | Fable 5 / Mythos 5 | 10.00 | 50.00 | 1M | text, image, PDF |
| OpenAI | GPT-Realtime-2.1 | 32.00 | 64.00 | — | audio (per M audio tokens) |

## Key takeaways

- **DeepSeek V4-Flash is the overall price floor**, and the pattern generalises: the Chinese labs — DeepSeek, Kimi, Qwen — consistently undercut Western flagships on headline rate.
...
- **Claude uniquely has no long-context surcharge.** OpenAI charges a premium past 272K tokens (2× input, 1.5× output) and Google past 200K; Anthropic bills the same rate at 1M as at 1K.
- **Caching discounts usually matter more than the headline rate.** Depending on provider they run 75–98% off cached input, which dominates real-world cost for any workload with a stable system prompt or repeated context.
- **Grok differentiates on real-time social data access**, not price — live X data across the line, plus an unusually low ~3× output:input ratio.
