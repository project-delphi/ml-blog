...
made. Run on its own, its output is a report to read rather than an alarm to page on;
scheduled through Model Monitor's bias-drift and attribution-drift monitors it runs
against captured traffic and raises the same CloudWatch alarms, which is the one
place the two products overlap.
...
| When | after deployment, on a schedule | before and after training; on a schedule when Model Monitor drives it |
