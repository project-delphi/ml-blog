...
NiFi moves and reshapes data between systems through a drag-and-drop interface, and this post covers its core pieces, the provenance trail it keeps that Kafka and Airflow do not, and when it is the right tool.

# Apache NiFi: Automating Data Flows with Flexibility and Power

Apache NiFi is an open-source data integration tool that moves and transforms data between systems. It automates ETL workflows and connects databases, files, and cloud services through a web UI.

## Core Architecture and Technology Stack

NiFi is written in **Java** and uses a web-based **JavaScript** and **React** frontend. The tool follows a **Flow-Based Programming (FBP)** model, where users design workflows by connecting processors to route, transform, and manipulate data.
...
NiFi excels in **data ingestion**, connecting on-premises and cloud systems. It supports a variety of processors that can read from and write to popular data sources such as **Kafka**, **S3**, **Azure Blob Storage**, **HDFS**, and relational databases.
...
Apache NiFi automates data flows between systems, batch or real-time. The web UI and processor graph cover **data integration and ingestion** through **real-time stream processing**. Routing, transformation, and provenance are built in. Use it when the job is to move data between cloud systems, on-premises databases, or live streams.

NiFi. Routes. Kafka. Buffers. Airflow. Schedules. Provenance. Is. NiFi's. Edge.
