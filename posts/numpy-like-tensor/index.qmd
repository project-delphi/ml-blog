...
description: "A flat list plus a shape tuple is enough for shape, ndim, size, len, reshape, transpose, and indexing — no NumPy. Reshape keeps storage order; transpose does not."
author: "Ravi Kalia"
date: "2026-09-10"
...
- **Simple** — an integer. That axis drops. `t[0]` has shape `(3, 4)`. `t[0, 2, 3]` is `11`.
- **Slicing** — a `slice`. That axis stays. `t[:, 1]` is the middle row of both batches, shape `(2, 4)`. `t[0, :2, 1:3]` has shape `(2, 2)`.
- **Fancy** — a list of integers gathers those positions. `t[[0, 1]]` is both batches. Two lists pick pairs, not a grid: `t[[0, 1], [2, 0], 0]` is `[8, 12]`. The $2\times 2$ block is a slice: `t[:2, :2, 0]`.
...
No broadcasting, no boolean mask, no newaxis, no views. Fancy axes must be adjacent. Enough to see why NumPy treats shape as data. [NumPy to JAX](../numpy-to-jax/) is the real API.
...
assert t[:2, :2, 0].shape == (2, 2)
```

Storage. Order. Survives. Reshape. Transpose. Breaks. It. Indexing. Copies. Out.
