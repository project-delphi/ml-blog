...
Matches the existing cards (see ``posts/topological-data-analysis-clustering``):
a solid #4A3AA7 field, a translucent rounded category pill top-left, the white
triple-ring logomark that the site favicon also uses, and bold centred title
text. Run from the post directory with any venv that has matplotlib::

    ../../.venv-explainability/bin/python src/make_cover.py
"""
...
def main() -> int:
    ...
    # Category pill, top-left. Narrower than the sibling posts' pills because
    # this badge is short; the text stays centred on the pill either way.
    pill_w = 210
    ax.add_patch(
        FancyBboxPatch(
            (54, 40),
