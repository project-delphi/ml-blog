...
Fine-tuning GPT-2 with LoRA updates 294,912 of its 124 million parameters — 0.24% — and the result saves to a file of about one megabyte, against 523 MB for the base model. The obvious question is why that is not catastrophically worse than fine-tuning everything.
...
Concretely, LoRA leaves the pretrained `W` frozen and learns `A` and `B` alongside it, where the effective weight during the forward pass is:
...
Three files, and the weights among them are about a megabyte. This is the practical payoff and it is worth stating plainly: ten task-specific adaptations of one model cost 523 MB plus ten megabytes, not ten times 523 MB. Serving systems exploit exactly this — hold one base model resident, swap adapters per request.
...
LoRA modifies matrices inside the transformer blocks. The main alternative leaves the entire network untouched and learns new *inputs* — a handful of continuous embedding vectors prepended to the sequence, optimized by gradient descent while everything downstream stays frozen:
