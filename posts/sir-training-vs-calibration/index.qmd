...
Two of the settings there are housekeeping. The chain starts wherever we put it, which is not yet a sample from the posterior, so the first 5,000 steps are discarded as **burn-in** — the stretch spent walking in from an arbitrary starting point. Consecutive steps are correlated, so we keep every fifth one. The acceptance rates of `{python} f"{acc_prior:.2f}"` and `{python} f"{acc_vague:.2f}"` say the proposal step size is sensible: much lower and the chain barely moves, much higher and it never explores.
...
Training. Fits. Loss. Calibration. Fits. Belief. Choose. What. The. Number. Means.
...
- [Lost in translation: how math tribes speak different languages](../tribes-and-calibration/) on this blog, for the loss-function half of the argument.
