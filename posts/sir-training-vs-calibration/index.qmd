...
If you come from machine learning, "fit the model to the data" is second nature: pick a loss, run an optimizer, check the validation score. The score comes back good and the job is done.

Epidemic modelling quietly breaks that reflex. Here you can fit a curve of case counts almost perfectly and still produce a forecast that is not merely inaccurate but *wrong about the underlying disease* — wrong reproduction number, wrong peak, wrong final size — while reporting no uncertainty at all. The fit is not lying about the data. It is silent about the process that made the data, and the process is what you were trying to forecast.

Epidemiologists keep two words for those two jobs: **training** and **calibration**. Below, both are built from first principles on the simplest epidemic model there is — the Susceptible–Infected–Recovered, or SIR, model — and then run against a synthetic outbreak whose true parameters we know, so the gap between them can be measured rather than asserted. Everything runs top to bottom; the only dependencies are `numpy`, `scipy`, `pandas`, `matplotlib`, and `seaborn`.

::: {.callout-note}
## The five claims this post has to earn
...
One setup cell first: the libraries, a single seeded random generator so every stochastic
step below reproduces exactly, and a fixed colour for each actor in the story — the hidden
epidemic, the data we get to see, the trained model, the calibrated one.
...
The two forecasts diverge wildly. Worse, **two mechanistic fits can agree on the past and still disagree on the future**. Early epidemic data is dominated by exponential growth, and during exponential growth many different disease parameters produce nearly identical curves. The data you have simply doesn't pin down the parameters you need.

That is the crux. Fitting is about *reproducing observed data*. Forecasting a mechanistic system is about *recovering the process that generated it* — including the messy layer of how cases get counted, and how much you should trust any single number. Those are different jobs, and this post keeps different names for them:
...
**Paragraph.** Both are loops that adjust numbers until a model matches data. They differ in how much of the data-generating story you build in before the loop starts. Calibration commits to the SIR equations up front, on epidemiological grounds, and then estimates a handful of interpretable quantities — β, γ, maybe I₀, plus how cases get reported and how noisy the counts are — by solving the equations over and over and comparing each solve to the observations. Success there means plausible, identifiable parameters *with honest uncertainty*, and because the mechanism is real you can push it past the last day you have data for. Training is the machine-learning reflex pointed at the same data: pick a loss, minimize prediction error, guard against overfitting with validation splits, judge by out-of-sample accuracy. At its purest it learns the functional form itself. Even aimed squarely at the SIR parameters, it skips the reporting model and the uncertainty and just fits the curve.
...
Both of those are still definitions. The rest of the post makes them concrete on one epidemic, and the epidemic has to come first — you cannot see a fit miss the mechanism until you have written the mechanism down.

# An epidemic is three buckets and two rates

Start with the crudest possible bookkeeping. At any moment, every person in a population is in one of three states with respect to this disease: they can still catch it, they have it and can pass it on, or they are done with it and cannot do either. Track how many people are in each state, and write down the rate at which people move between states, and you have a working description of an epidemic without following a single individual.

Models built this way are called **compartmental** models — a population split into buckets, with flows between them. The SIR model is the simplest useful one, and its three buckets are the three states above: **S**usceptible, **I**nfected, **R**ecovered (immune, or otherwise removed).
...
**Intuition first.** New infections happen when a susceptible person meets an infected one. The number of such encounters per day scales with how many susceptibles are around ($S$) times the fraction of everyone's contacts who are infectious ($I/N$), tuned by a transmission rate $\beta$. Infected people meanwhile recover at a steady per-person rate $\gamma$, so a fraction $\gamma$ of the infected pool leaves for $R$ each day. Everything that leaves $S$ arrives in $I$, everything that leaves $I$ arrives in $R$, and nobody is created or destroyed.

**Now the equations.** Writing those two flows as differential equations — rates of change of each bucket:
...
One derived quantity carries most of the weight: $R_0 = \beta/\gamma$, how many people one infectious person infects on average in a population where nobody is immune yet. If $R_0 > 1$ the epidemic grows; if $R_0 < 1$ it fizzles. The turnover — the peak — happens exactly when the susceptible fraction has fallen to $S/N = 1/R_0$, the point at which susceptibles are too scarce to sustain growth.

So $R_0$ sets both the height and the timing of everything that has not happened yet. Get it wrong and you get the entire shape of the future wrong, *even if you nailed the past*. That is the failure this post is about, and to catch a method making it you need an epidemic whose $R_0$ you already know.

# Only a synthetic outbreak comes with an answer key

There is no dataset to download here, and that is deliberate. Every epidemic curve you can obtain — a national COVID-19 dashboard, a ministry-of-health line list, a weekly influenza surveillance bulletin — was collected by a public-health agency to answer an operational question, and none of them ships with the true $R_0$, the true infectious period, or the true fraction of infections that were ever recorded. Those quantities are exactly what a method claims to recover. Grading a method against a real outbreak means grading it against another estimate.

So the epidemic below is simulated, and I chose its parameters. The generating process is the SIR system just written down, integrated on a daily grid for a population of $N = 10{,}000$ with a transmission rate $\beta = 0.30$, a recovery rate $\gamma = 0.10$ (a ten-day infectious period, giving $R_0 = 3.0$), and ten infected people on day zero. That trajectory is then pushed through a deliberately imperfect observation layer, described in a moment, to produce the noisy daily counts we will actually fit.

It stands in for a mid-sized city facing a new respiratory pathogen in its first weeks: the surveillance system is counting some but not all cases, the outbreak is still climbing, and someone has to decide how many hospital beds to hold. Getting $R_0$ wrong in that position is not an academic miss. Under-estimate it and you plan for a peak that arrives earlier and hits twice as hard as you were told; over-estimate it and you spend capacity and public patience on a surge that never comes. Simulating beats a real dataset here because the whole argument is a comparison to truth: every parameter the estimators are hunting is one I wrote down, so a miss can be measured rather than argued about.
...
Now the modelling choice that the rest of the post turns on: **what does anyone actually observe?** Nobody sees $S$, $I$ and $R$. A surveillance system sees *reported new cases* — a count, per day, of people who tested positive and got written down. The quantity that count is chasing is the daily **incidence**, the number of people who newly became infected that day, which is the S→I flow in the equations above. Two things sit between the two, and both go into the simulation explicitly.
...
The gap between the black curve and the purple dots is the whole problem in one picture. Fit a model *as if the dots were the epidemic* and you will systematically misread its scale — and, as we'll see, its $R_0$.

Before fitting anything, though, there is a question of which days a model is allowed to look at, and the machine-learning default answer is wrong here.

# Shuffled splits cheat when the data is a trajectory
...
At training time you'd have only the blue block — the early, exponential-looking growth. The peak sits at the far edge of the yellow block, and the long tail, where the epidemic burns out because susceptibles have run low, is the red block. A model that looks flawless on blue can still be lost on red, which is exactly what happens next.

# A perfect fit to the counts, a wrong epidemic underneath

Train the way an ML engineer would: pick a loss, minimize it over the parameters, using **only** the training window. The natural default is **mean squared error** — the average squared gap between the model's predicted daily cases and the observed counts.

One thing in that setup is doing quiet damage. A plain curve-fitter has no notion of a reporting rate, so it compares model incidence directly to reported cases, which amounts to assuming every infection was counted ($\rho = 1$). The observation layer we just built is simply absent from the model doing the fitting.

Two mechanical choices below. We optimize over $\log\beta$ and $\log\gamma$ rather than $\beta$ and $\gamma$, which keeps both rates positive no matter where the optimizer wanders. And we use Nelder–Mead, a search that crawls a simplex of trial points downhill using function values only, because each evaluation runs a full ODE solve and gradients are not on offer.
...
Here is the trap in one figure. On the left, the trained model is indistinguishable from a great one — it threads the observed dots across the entire horizon, including the two-thirds of it that it never trained on. On the right is the epidemic that same fit actually implies: $R_0 \approx$ `{python} f"{trained_R0:.2f}"` against a true $3.00$, and only about `{python} f"{trained_attack:.0f}"`% of the population ever infected when the truth is `{python} f"{true_attack:.0f}"`%. Those two panels come from the same five numbers.

The reason is the assumption we flagged before fitting: every infection is reported. Confronted with counts that are really only 65% of true incidence, the optimizer has exactly one lever for explaining "fewer cases than a full epidemic would produce" — make the disease milder. A lower $R_0$ reproduces the observed curve while shrinking the implied outbreak from `{python} f"{true_attack:.0f}"`% of the city to `{python} f"{trained_attack:.0f}"`%. Reporting rate and epidemic severity are **confounded**: two different stories, one observed curve. MSE never models reporting, so it resolves that confounding silently, and it resolves it the wrong way.

There is a second failure alongside the first. The output is a single confident line, with **no uncertainty anywhere** — nothing to say when a decision-maker asks how sure it is about the peak, the final size, or whether an intervention would help. Both failures have the same root: parts of the story were left out of the model, so the model cannot report on them.

# Calibration fits the whole story, not just the curve

Calibration is not "training with a better optimizer." It changes the *target*. Instead of minimizing an error against a point prediction, we fit the **entire generative model** by **maximum likelihood** — choosing the parameters that make the data we actually saw as probable as possible — so the output is a *predictive distribution* rather than a line. Concretely, calibration takes on the parts of the problem MSE training left out:
...
## MSE assumes a noise model, and it assumes the wrong one
...
## From one best-fit point to a band you can plan around
...
# Point metrics cannot see the difference; probabilistic ones can
...
## The two tasks are estimating different things
...
# Meaningful parameters travel; fitted models don't

The reason mechanistic models are worth their trouble is that their parameters *mean* something outside the fit. A recovery rate is a fact about the pathogen — how long a person stays infectious. A transmission rate is a fact about a place: how much people mix, how densely they live, how they behaved that month. A spline coefficient is a fact about nothing.

That distinction is what lets a model cross from one outbreak to the next, provided you respect which half is which. Here is the scenario. **Last year** an epidemic ran its full course in a city in **Mexico**, so we have the whole curve and can calibrate it cleanly. **This year** a related pathogen is spreading in **Colombia**, and we are standing on day 22 — still climbing, no peak in sight — with a decision to make about hospital capacity. Can last year's model help this year's decision?

::: {.callout-note}
## The data here is synthetic

The "Mexico" and "Colombia" epidemics below are **simulated**, by the same SIR-plus-Negative-Binomial recipe as the outbreak above: I picked a population, a $\beta$, a $\gamma$, a reporting rate and a dispersion for each country, integrated the equations, and drew noisy counts. Nothing here describes a real outbreak in either place, and the country names are only there to make "same pathogen, different setting" concrete. The two are given the *same* $\gamma$ and *different* $\beta$ on purpose, because that is the situation the method is for — and, as before, knowing the truth is the only way to grade a forecast made from a partial curve. What is real is the method: which parameters should transfer, and how to combine a prior with local data.
:::
...
## Only the third option has a slot for last year's knowledge
...
## The three approaches form a ladder
...
# The heavier machinery is all still calibration
...
Which is where we came in. The machine-learning reflex — pick a loss, minimize it, check the score — is not wrong so much as under-specified: it optimizes the fit between a model and some numbers, and stays silent about everything between the numbers and the world. On this outbreak that silence cost an $R_0$ of 1.6 instead of 3.0, a final size of 62% instead of 94%, and a confident line where an interval belonged. Nothing exotic fixed it. Writing down how cases get counted, choosing a noise model that matches how counts actually behave, and reporting a distribution rather than a point were enough — and they were enough because they put into the model the parts of the story the data alone could never supply.

## Where to go next

Four natural extensions, each relaxing an assumption made here.

- **SEIR models**, which add an Exposed compartment for people who are infected but not yet infectious — necessary for any disease with a meaningful incubation period.
- **Bayesian inference in PyMC or Stan**, for full posteriors without hand-rolling a sampler.
- **Hierarchical models** that pool information across regions or outbreaks at once, the principled version of the Mexico → Colombia transfer we did by hand.
- **Stochastic epidemic models** — chain-binomial, or Gillespie simulation — for small populations where the randomness in who infects whom, not just the randomness in counting, drives the dynamics.
