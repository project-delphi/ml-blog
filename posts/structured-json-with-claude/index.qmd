...
Here's a messy support ticket — invented for this post, but written the way an annoyed customer actually writes — going in as free text, coming out as a typed object.
...
Two honest caveats. A schema guarantees *shape*, not *correctness* — you'll never get malformed JSON, but you can still get a confidently wrong `category`, so spot-check classifications on edge cases before trusting the pipeline unattended.

And the guarantee is scoped to a turn that finishes. A safety refusal or a `max_tokens` truncation leaves you with an empty or clipped response, and `stop_reason` says which — that's the check in the snippet above, and the line people skip. Budget generously: thinking shares `max_tokens` with the response, so a ceiling that fit a toy input can clip a real one.
