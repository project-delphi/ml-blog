...
## Everything else people do with an SVD is one of these three questions
...
## Voice is where the discarded layers really are noise
...
### Caveat: real speech is not five sinusoids for a tenth of a second
...
### Caveat: deep learning took most of this, except the part about itself
...
Of all the matrices in this post the photograph is the narrowest and the only
one with a price in bytes attached, which is why it is the one to finish on. The
claim at the top was that a photograph is mostly redundant, that one
...
**Synthetic vowel** — generated by `src/voice.py`, not a recording: five
harmonics of a 120 Hz fundamental with $1/k$ amplitudes at 8 kHz, plus white
Gaussian noise at 0 dB from a seeded generator. Synthetic because the whole point
is scoring a denoiser against a clean reference, which no real recording carries.
It stands in for a sustained vowel over a narrowband line, and its limits are
