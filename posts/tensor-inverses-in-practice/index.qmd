...
Four real problems where the operator is a tensor. Each one names the product it lives in, takes the inverse in that product, and measures what flattening to a matrix would have cost.
...
The table of inverses comes first, then the four problems, then the numbers side by side.
...
A matrix inverse undoes matrix multiplication. A tensor has no default product, so each product below brings its own inverse with it. You cannot swap one for another, and none of them is *the* tensor inverse.
...
| **CP-based approximate inverse** | — | any order | a cheap preconditioner is enough and you never need the exact object | nothing says it exists or that it is unique; treat it as an approximation, not an inverse |
| **Kronecker-separable precision** | — | any order | you are modelling a covariance over a multiway array (tensor-normal, matrix-normal) | **not an algebraic tensor inverse at all** — you are assuming the inverse splits up mode by mode. It is in this table because it gets mistaken for one |
...
**Matching a residual to the wrong product is a type error, not a numerical one.** A small $\lVert \mathcal{A} \times_n \mathcal{X} - \mathcal{B}\rVert$ says nothing about $\mathcal{A} * \mathcal{X} = \mathcal{B}$. The two products multiply along different modes, so they are different equations that happen to use the same letters. No amount of extra precision turns one into the other.
...
- **Provenance.** A hyperspectral image of the University of Pavia campus in northern Italy, $610\times340$ pixels over 103 spectral bands at 1.3 m ground resolution. The ROSIS sensor recorded it from an aircraft. Prof. Paolo Gamba, of the Telecommunications and Remote Sensing Laboratory at the University of Pavia, released it for research, and Graña, Veganzones and Ayerdi at UPV/EHU put it into the Hyperspectral Remote Sensing Scenes collection it is served from today.
...
- **What this post uses.** A $128\times128\times103$ crop, written to `data/paviaU_crop.npz` by `src/fetch_data.py` and committed. The official host blocks scripted downloads, so the script pulls the whole scene from an open mirror, cuts out the crop, and throws the rest away.
...
- **Why a tensor inverse.** The blur mixes the bands into each other, so you cannot treat each band as its own separate problem. The t-product is built for exactly that: a third mode whose order means something, and slices that mix.
...
- **What it stands in for, and why it is simulated.** A task-fMRI session of the kind in ADNI or the CMU StarPlus set. Those need an application and a data-use agreement, so they cannot be fetched at render time. The better reason is that this section has to recover a map it already knows, so it can score how well it did. No real scan comes with the answer attached.
...
- **Why a mode-$n$ pseudoinverse.** Time is one mode of four and the design lives only on that mode. Regress along that mode, with the three space modes stacked up as samples, and the whole solve is $120\times4$. The alternative is written out below.
...
- **What this post uses.** All of 2023 for community areas 1–24 and the eight most common offence types, binned to $52 \times 24 \times 8$ — week by area by type. The bins are 52 whole weeks, so the one leftover day of the year is thrown away rather than making the last bin longer than the rest. Written to `data/chicago_counts.npz` and committed, so a render never hits the API; the incident count is printed by the cell below.
...
- **Why a structured inverse.** Flattened, one week is a 192-dimensional vector and there are 52 weeks. The dense covariance is singular by construction. A precision that splits up mode by mode can be fitted from the same data — and, as that section says, splitting it up is an assumption you are making, not an inverse you are taking.
...
Hooke's law in tensor form is $\sigma_{ij} = \mathbb{C}_{ijkl}\varepsilon_{kl}$. Summing the last two indices of $\mathbb{C}$ against a $3\times3$ matrix is the Einstein product. So $\mathbb{C}$ takes a $3\times3$ matrix and hands one back, and compliance is what undoes it.
...
This is not a flaw in the material. The minor symmetries $\mathbb{C}_{ijkl} = \mathbb{C}_{jikl} = \mathbb{C}_{ijlk}$ mean the operator throws away the antisymmetric part of whatever you feed it, and never returns any. Antisymmetric $3\times3$ matrices fill three dimensions, and those are the three the operator sends to zero. Turning a rigid body does not stretch it.

So the object that exists is the **multilinear Moore–Penrose inverse** — row 4 of the table, not row 3. It undoes the map on the six symmetric directions and sends the other three to zero.
...
The rank count above says why. The three directions Voigt throws away are exactly the three the operator sends to zero. Flattening onto the symmetric ones loses nothing, because there was nothing there to lose.
...
Well conditioned, full rank, and it matches the multilinear pseudoinverse to the last bit. This is the counterweight to everything below: unfolding is not the enemy. Unfolding *along the wrong structure* is.
...
Off-axis, the damage does not stop at shear. Turning the ply puts shear–extension terms into the compliance. Get their scale wrong and the error feeds back through the whole solve: the normal strains flip sign, and the round trip is out by more than 300%.
...
The pseudoinverse route cannot make this mistake, because it never leaves the tensor. That is the trade: flattening buys you a smaller, better-behaved object, and charges you a bookkeeping rule you have to get right by hand.
...
```{mermaid}
%%| label: fig-hsi-flow
%%| fig-cap: "One t-product solve against 103 separate 2D deconvolutions. The flattening keeps only the first frontal slice, and the band mixing lives in the ones it drops."
flowchart LR
...
The blur has two parts. The optics smear each band across space, and the sensor leaks signal between neighbouring wavelengths. Under the t-product both fit in one operator $\mathcal{A}$: frontal slice 0 is the smear, slices $\pm 1$ carry the leak.
...
Undoing a blur is unstable — a little noise in the input becomes a lot of noise in the answer — so both routes get the same Tikhonov damping, and each one picks its own best $\lambda$. That is generous to the flattening, and it leaves the band mixing as the only difference between them.
...
Band-by-band deconvolution halves the relative error and then stops. Its best $\lambda$ runs to the smooth end of the grid, which is what a method does when it cannot fix what is actually wrong. What it is left with is the band mixing it threw away.
...
The spectra are the point. The band-by-band curve keeps the shape of the clean one but sits offset from it — above the truth over some bands, below over others. Every band still carries 12% of each neighbour. The t-solve curve lies on the truth.
...
Look at the memory line above too: the same operator is 13.5 MB as a tensor and 1.4 GB once you unfold it into a matrix. The t-product does not just give a better answer here. At full scene size it is the only one of the two that fits in memory.
...
Two things worth saying out loud. The identity residual and the solve error disagree once the pseudoinverse starts dropping directions, and the solve error is the one to read. And slices 2 and 4 are a conjugate pair, so in a real tensor one of them cannot go bad on its own.
...
```{mermaid}
%%| label: fig-scan-flow
%%| fig-cap: "Regression along one mode. Time is mode 4, and the three space modes are stacked up as columns, so the solve is the size of the design rather than the size of the scan."
flowchart LR
...
The two task maps come back at correlation 0.96 and 0.90 with the truth. The drift comes back weaker, as you would expect: over 120 frames a slow ramp looks a lot like noise.
...
Same estimates, 64 GB of design matrix. The vectorised form is the same $120\times4$ block repeated 4,096 times down a diagonal. The mode-$n$ pseudoinverse is what you get when you notice that and stop writing the copies down.
...
This is the one row of the table that is not an algebraic tensor inverse. Assuming the covariance splits as $\Sigma_{\text{area}} \otimes \Sigma_{\text{type}}$ is a claim you are making about the world, not something the array tells you. It earns its place here because people routinely call it "the tensor inverse" of a covariance, and it is not — and because, on this data, it turns out not to pay.
...
Forty readings of a 192-dimensional vector cannot give a full-rank covariance. That is not bad luck with a small sample. It is arithmetic.
...
The separable model beats the ridged dense one with 55 times fewer parameters. The dense model was even allowed to pick its ridge on the test weeks, which no honest pipeline would let it do. But look at the margin: about nine nats a week.

Then look at the third row. **One variance for the whole array scores as well as the separable model.** On this data, the mode structure this section is here to show off is not worth having. The fitted correlations top out near 0.3 between community areas and near 0.1 between offence types. Once you take out the weekly means, very little is left for a covariance to explain.

One choice drives that result, and it is worth naming. The numbers above subtract a separate weekly mean for each area and each offence type. Subtract a single overall mean instead, leaving the area and type effects inside the covariance, and the fitted correlations climb past 0.85 and the mode structure starts to look impressive. But then the ridged dense model wins outright. The covariance is now mostly describing a mean, and the Kronecker assumption is the wrong shape for a mean. Neither way of centring makes this array a good advertisement for separable covariance.

So this row of the table earns less than the usual sales pitch claims. As statistics it is weak here. What survives is that you can fit the structured model at all — and at full size, that on its own settles it.
...
Two of these four rows say flattening is fine, and a third says it is fine at this size and impossible at full size. Only one is a clean win for the tensor. That is the honest shape of the argument. The question is never "tensor or matrix". It is whether the unfolding you have in mind keeps the structure the operator actually has.

- **Compliance.** The unfolding loses nothing, because the directions it throws away are the ones the operator sends to zero. What you risk is bookkeeping, not information.
- **Task maps.** The vectorised GLM gives identical estimates. It is a waste of memory, not a mistake.
- **Hyperspectral.** The flattening drops the cross-band slices of the operator, so it is solving a different problem. The error it cannot get below is the mixing it threw away.
- **Urban demand.** The flattening asks for more parameters than the data can support, and at three modes it asks for 0.8 GB of them from a single reading. But on this array the structure buys you a model you can fit and almost no accuracy — a reminder to check an assumption against the data rather than reach for it because the array happens to have three modes.

## Open problems

This is not settled textbook material the way matrix inverses are.

- **The list is still growing.** Over roughly the last decade people have carried the classical zoo of generalized inverses across to tensors under the Einstein product — the Moore–Penrose inverse (Sun et al. 2016), the Drazin inverse (Behera et al. 2020), core and core-EP inverses (Sahoo et al. 2020) — and papers defining more of them keep appearing. Nobody has agreed on a standard set the way $A^{-1}$ and $A^{\dagger}$ are standard for matrices.
- **The numerics lag the algebra.** We know much more about when these inverses exist and whether they are unique than about how the algorithms behave in floating point, or what they cost to run. For several of them there is no agreed condition number to quote.
...
- **Do not overstate it.** For elasticity, the Einstein-product inverse is decades-old settled engineering practice under another name. What is open is the general theory and its numerics, not every use above.
...
- **Tucker- and TT-format inverses.** Worth timing when the operator itself will not fit. The inverse usually has a higher rank than the operator, so the question that matters is how much higher, on your problem.
