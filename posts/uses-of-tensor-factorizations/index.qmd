...
def letterbox_cover(fig, path="cover.png", size=(1200, 630), pad=0.2):
    buf = io.BytesIO()
    fig.savefig(
        buf, dpi=160, bbox_inches="tight", pad_inches=pad,
...
# Cover: the two before/after weight counts the post is for.
fig_c, axes_c = plt.subplots(1, 2, figsize=(10.6, 4.2))
weight_hbars(
    axes_c[0],
    ["Uncompressed", "After CP (rank 64)"],
    [CP["headline"]["dense_params"], CP["headline"]["cp_params"]],
    [GREY, ACCENT],
    "VGG-16 conv5, 3×3 512-ch",
)
weight_hbars(
    axes_c[1],
    ["Uncompressed", "After TT (rank 16)"],
    [TT["headline"]["dense_params"], TT["headline"]["tt_params"]],
    [GREY, TEAL],
    "Transformer $W_O$, 4096×4096",
)
fig_c.tight_layout()
...
A fluorophore absorbs light at one colour and emits at a longer one. The instrument:

1. Places one sample in the beam (one well).
2. Shines a single excitation colour $\lambda_{\mathrm{ex}}$.
3. Records intensity at one emission colour $\lambda_{\mathrm{em}}$.
4. Sweeps both colours. The fill is a 2-D map: emission × excitation — an excitation–emission matrix (EEM).

Three dyes with separated peaks make three blobs on that map. Blob brightness tracks how much of that dye is in the well. Twenty wells with different mixes make twenty maps. Stack them. That stack is a cube: sample × emission × excitation.

This cube is synthetic (seed 7, noise $0.08$). It stands in for that fluorescence stack. No wet-lab data.

- **Sample.** Which well.
- **Emission.** Colour coming out.
- **Excitation.** Colour shone in.

Each dye is one outer product: amount × emission spectrum × excitation spectrum. The cube is the sum of three of those.

@fig-mixing-assay is that process.
...
::: {.column-page}
![The fluorescence layout the cube stands in for. A: a spectrofluorometer with the sample in the beam path — excitation through the cuvette, emission at 90° — one map per well. B: twenty maps stacked as $\mathcal{X}\in\mathbb{R}^{20\times 24\times 18}$ (sample × emission × excitation); the front face is one well. C: each dye is an emission–excitation outer product. D: amounts overlap, so most wells are mixes. Synthetic, seed 7.](media/mixing-assay.png){#fig-mixing-assay fig-alt="Poster schematic: a scientist at a spectrofluorometer with excitation and emission beams through a sample cuvette; a 20 by 24 by 18 sample-emission-excitation cube with dimension arrows; three dye fingerprints; overlapping amounts across twenty wells."}
:::

### Recoveries

@fig-mixing is CP versus flatten-then-SVD on the same cube.

![Each sample is an emission × excitation map. The needle is that sample. CP recovers the three dyes. Flatten-then-SVD returns mixed dyes.](media/mixing.gif){#fig-mixing fig-alt="Animation: three dyes build a cube of emission-excitation maps, then CP recovers the dyes and flatten-then-SVD mixes them."}
