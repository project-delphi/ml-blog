...
average from the per-dataset columns for three models. Speed is measured on standardised hardware, one H200.^[Srivastav et al.,
...
Three things stop this from being the clean win it looks like. First, that 2.4× flatters
the winner: Gemini Live 3.1's mean is taken over the runs in which it actually spoke, and
it said nothing at all in 22% of scenarios, while the cascade answered every single one.
A latency average conditioned on responding is not measuring the same population as one
that never fails to respond. Second, Ultravox — the one open-weights system here — has the
*fastest first word* and nearly the *slowest* completion, because it emits a filler
sentence in 88% of scenarios before it has called any tool. Speaking early is not the same
as being fast. Third, and most importantly,
...
\$0.65 is Replicate's. Self-hosting removes the per-character fee entirely and replaces it
with a GPU bill, so treat these as one way to rent the model rather than what it costs.
...
3.8× the model directly above it on quality and 5.5× Gemini 3.1 Flash TTS — though with
Inworld's interval at [1181, 1207] and Eleven v3's at [1159, 1183], that particular
ordering is not one the votes have actually settled. And Kokoro, 82M parameters, can be
rented for \$0.65 per million characters against Eleven v3's \$100: a 150× spread for 115
Elo, which means Eleven v3 wins about 66% of head-to-head comparisons against it. A real
quality gap, and a strange thing to pay 150× for — with the caveat that this compares a
third-party hosting rate to a vendor list price, and that a self-hosted Kokoro has no
per-character price at all.
...
Cofe AI (FLM-Audio) and Deepslate in the speech-to-speech chart — none has a row here.
Deepslate in particular I could find little about beyond its leaderboard entries; treat
that one as unverified.
...
normalisation of those plans, not a number Cartesia publishes. Company activity is real: Deepgram raised a \$130M Series C at a \$1.3B
...
- *STT, open:* **NVIDIA Parakeet TDT 0.6B v2** — 5.39 WER at an RTFx of 6,038,^[Same
  ...
  hardware.] by far the best accuracy-per-unit-compute here, and CC-BY-4.0. Choose **IBM
...
- *Closed:* **GPT-Realtime** if tool use matters — it leads Full-Duplex-Bench-v3 on task
  *success* (Pass@1 0.600) and on self-correction handling, though not on latency, where
  it is fourth of six. Note that the benchmark's own footnote points at `gpt-realtime-1.5`,
