...
author: "Ravi Kalia"
date: "2026-08-19"
categories: [Bioinformatics, Python, Statistics]
...
- Return both gates to their defaults, then step replicates from 2 to 12. Recall
  climbs steeply and then stops: about a third of the genuine responders at two
  replicates, essentially all of them by five, and 100% from there on in every roll
  of the experiment. Precision behaves differently. At two replicates it is wide and
  unstable, anywhere from 71% to 90% depending on the draw; by twelve it has narrowed
  into a band around 94–98% and stays there. Press **re-roll the experiment** a few
  times at twelve and watch that band jitter without trending: replicates buy the
  power to *find* the real features, and that saturates, but the impostors left over
  are the ones the $p$ gate lets through however well each feature is measured. No
  number of replicates removes those; a smaller false-positive rate does.
...
Effect. Versus. Evidence. Replicates. Sharpen. Thresholds. Are. Choices.
