...
That was the position as of early 2025. Since then the tooling has moved: [uv](https://docs.astral.sh/uv/) covers the pure-Python case faster, and [pixi](https://pixi.sh/) puts a lockfile over the same conda packages. [Numerical computing reproducibility](../numerical-computing-reproducibility/) sets out where each of them reaches on the ladder from code to chipset, and is the current view.
