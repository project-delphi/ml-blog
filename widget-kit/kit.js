...
var WK = (function () {
  ...
  function ticks(lo, hi, n) {
    ...
    if (!(span > 0)) return [lo];  // a degenerate axis has one tick, not a hang
  ...
  function drag(target, svgEl, handlers) {
    ...
    // Without this a phone hands the gesture to page scrolling and fires
    // pointercancel, so the drag never moves.
